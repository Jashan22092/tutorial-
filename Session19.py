...
@web_app.route("/register_vet",methods=['POST'])
def register_vet():
...
def update_pet_in_db():
    pet_data_to_update = {
        ...
        'age': int(request.form['age']),
        'gender': request.form['gender'],
        # Add other fields you want to update for the pet
    }

    if len(pet_data_to_update['breed']) == 0:
        return render_template('error.html', message="Breed cannot be empty")
    ...
    return render_template('success.html', message="Pet updated successfully")


@web_app.route("/delete-pet/<id>", methods=['GET', 'DELETE'])
def delete_pet(id):
...
@web_app.route("/delete-consultation/<id>", methods=['GET', 'DELETE'])
...
def update_consultation_in_db():
    ...
    if len(consultation_data_to_update['name']) == 0 or len(consultation_data_to_update['problem']) == 0:
        return render_template('error.html', message="Name and Problem cannot be empty")
